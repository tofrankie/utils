 ...
 */
export function convert2pinyin(sourceStr) {
  // 目标数据
  let targetStr = ''
  // 匹配中文
  const cnReg = /[\u4E00-\u9FA5]/
  // 匹配数字和英文
  const enReg = /[a-z0-9]/i
  // 保留英文和数字
  const keep = true
  ...
  for (let i = 0, len = sourceStr.length; i < len; i++) {
    ...
    if (keep && enReg.test(str)) {
      ...
      const searchResult = searchPinYin(str, PIN_YIN)
      if (searchResult)
 ...
 */
function searchPinYin(str, data) {
  for (const key in data) {
    if (Object.prototype.hasOwnProperty.call(data, key) && data[key].includes(str)) {
      return key
    }
  }
